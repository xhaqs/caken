...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/saved-chats') {
      return handleSavedChats(request, env, url);
    }
...
async function handleSavedChats(request, env, url) {
  try {
    if (!(await checkAuth(request, env))) return unauthorized();

    if (request.method === 'POST') {
      const body = await request.json();
      const r = await fetch(DB + '/caken/savedChats.json', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      });
    ...
    if (request.method === 'DELETE') {
      ...
      await fetch(DB + '/caken/savedChats/' + key + '.json', { method: 'DELETE' });
      return new Response(JSON.stringify({ ok: true }), {
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
      });
    }
